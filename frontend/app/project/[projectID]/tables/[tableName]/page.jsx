...
export default function TableDataGridPage() {
  ...
  // Returns true if this column should be EXCLUDED from the insert form.
  // Only skip columns whose value will be generated by the DB itself:
  //   - gen_random_uuid() / uuid_generate_v4() defaults  → DB generates UUID
  //   - nextval(...) defaults                            → DB generates serial/sequence
  // PKs WITHOUT a default (e.g. integer PK, varchar PK) stay visible — user must supply a value.
  const isAutoColumn = (col) => {
    if (!col) return false;
    const def = col.column_default || "";
    return (
      def.includes("gen_random_uuid") ||
      def.includes("uuid_generate") ||
